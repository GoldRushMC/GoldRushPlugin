...
import com.goldrushmc.bukkit.trainstation.exceptions.NoExitAssignedException;
...
/**
 * This is the types that is meant for TWO rail tracks. One is for public transport, the other is for goods.
 * <p/>
 * This types class will be the most widely used, as it encompasses public and good transport, which is how the gold rush map was built...
 *
 * @author Diremonsoon
 */
public class HybridTrainStation extends TrainStation {
        ...
        switch (type) {
            case MINECART: {
                if (this.publicTrain == null) {
                    owner.sendMessage("There is no public trainstation to buy from at this time.");
                    return false;
                }
                    ...
                    if (mm instanceof MinecartMemberRideable) {
                        if (mm.getProperties().getOwners().isEmpty()) {
                            ...
                            owner.sendMessage("You just bought public cart: " + ChatColor.YELLOW + "(" + mm.getIndex() + ")"
                                    + " from trainstation: " + this.publicTrain.getProperties().getTrainName());
                            return true;
                        }
                    }
                }
            ...
            case MINECART_CHEST: {
                if (this.goodsTrain == null) {
                    owner.sendMessage("There is no transport trainstation to buy from at this time.");
                    return false;
                }
                    ...
                    if (mm instanceof MinecartMemberChest) {
                        if (mm.getProperties().getOwners().isEmpty()) {
                            ...
                            owner.sendMessage("You just bought transport cart: " + ChatColor.YELLOW + "(" + mm.getIndex() + ")"
                                    + " from trainstation: " + this.goodsTrain.getProperties().getTrainName());
                            return true;
                        }
                    }
                }
    ...
    public boolean sellCart(Player owner, EntityType type) {
        switch (type) {
            case MINECART: {
                if (this.publicTrain == null) {
                    owner.sendMessage("There is no public trainstation to sell from at this time.");
                    return false;
                }
                for (MinecartMember<?> mm : this.publicTrain) {
                    if (mm instanceof MinecartMemberFurnace) continue;
            ...
            case MINECART_CHEST: {
                if (this.goodsTrain == null) {
                    owner.sendMessage("There is no transport trainstation to sell from at this time.");
                    return false;
                }
                for (MinecartMember<?> mm : this.goodsTrain) {
                    if (mm instanceof MinecartMemberFurnace) continue;
    ...
    public boolean pushQueue() {
        ...
        if (this.publicTrain != null) {
            ...
            //Clear the focus from the trainstation.
    ...
    public void addDepartingTrain(MinecartGroup train) {
        for (MinecartMember<?> mm : train) {
            ...
            //Public transport trainstation
            if (this.publicStopBlocks.contains(mm.getBlock())) {
                this.publicTrain = train;
            }
            //Goods transport trainstation
