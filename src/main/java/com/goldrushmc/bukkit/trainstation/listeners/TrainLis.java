...
public class TrainLis extends DefaultListener {
    ...
    /**
     * Checks to make sure that the groups connecting are of the same trainstation, and if they aren't, cancel the event.
     *
...
//	}
	...
	public void onGroupBreak(GroupRemoveEvent event) {
		...
		String trainstation = event.getGroup().getProperties().getTrainName();
		if(TrainFactory.trains.containsKey(trainstation)) {
			TrainFactory.trains.remove(trainstation);
		}
