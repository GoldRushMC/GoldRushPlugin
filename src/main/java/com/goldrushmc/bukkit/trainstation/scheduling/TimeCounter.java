...
public class TimeCounter implements Runnable {
    ...
    public TimeCounter(JavaPlugin plugin, World world) {
        ...
        if (fc != null) {
            ...
            //If ANY time values are 0, we reset them to 3.
            if(times.containsValue(0L)) {
                ArrayList<String> toDefault = new ArrayList<>();
                for(String s : times.keySet()) {
                    if(times.get(s) == 0) toDefault.add(s);
                }
                //Remove the current mapped bad values
                for(String remove : toDefault) times.remove(remove);
                //Add the default value, 3.
                for(String add : toDefault) {
                    times.put(add, 3L);
                }
            }
        }
    ...
    public void run() {
        ...
        //For measurement of time accrual -- This will be used for updating of signs.
        if(Math.abs(timeCount - publik) == times.get("Public")) {
            publik = timeCount;
        }
        if(Math.abs(timeCount - transport) == times.get("Transport")) {
            transport = timeCount;
        }
        ...
        //Update the departure times for each types. This will be a bit expensive. Every minute, we update ALL time signs.
        if (update % 1200 == 1) {
...
//        }
    ...
    public void onUpdateTick() {
        ...
        for (TrainStation station : stations) {
            ...
            if(add){
                //TODO Need to add a Hub types class, and add logic to add it to scheduling.
