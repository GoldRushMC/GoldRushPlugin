...
//		BlockFace leftDir = TrainTools.toTheLeft(TrainTools.getDirection(player));
    ...
    public static void addCart(Player owner, String trainName, EntityType cart) {
        ...
        if (trainToLink.isMoving()) {
            owner.sendMessage(ChatColor.RED + "The train is currently moving. Please wait until it has arrived at a station.");
            return;
        }

        //If the train size is 15 long, it is the max size, and we should not add more.
        if (trainToLink.size() == 15) {
            owner.sendMessage("The train is full! Please wait for the next train to come.");
            return;
        }
    ...
    public static void removeCart(String trainName, Player owner, EntityType cartType) {
        ...
        if (train == null) {
            owner.sendMessage("It appears this train does not exist.");
            return;
        }
